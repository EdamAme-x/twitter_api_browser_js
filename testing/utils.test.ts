...
Deno.test("expect pickFirstItem to return an error if no items are provided", () => {
  assertEquals(pickFirstItem([]) instanceof Error, true);
});

Deno.test("expect pickFirstItem to return an error if multiple items are provided", () => {
  assertEquals(pickFirstItem([1, 2]) instanceof Error, true);
});

Deno.test("expect removeNullRecursively to remove null properties from object", () => {
  const obj = { a: 1, b: null, c: "value" };
  const result = removeNullRecursively(obj);
  assertEquals(result, { a: 1, c: "value" });
});

Deno.test("expect removeNullRecursively to remove null from nested objects", () => {
  const obj = {
    a: 1,
    b: null,
    c: {
...
Deno.test("expect removeNullRecursively to remove null from nested arrays", () => {
  const obj = {
    a: [1, null, 3],
    b: null,
    c: [
...
Deno.test("expect removeNullRecursively to return primitive values as-is", () => {
...
Deno.test("expect removeNullRecursively to return null and undefined as-is", () => {
  assertEquals(removeNullRecursively(null), null);
  assertEquals(removeNullRecursively(undefined), undefined);
});

Deno.test("expect removeNullRecursively to handle empty objects and arrays", () => {
  assertEquals(removeNullRecursively({}), {});
  assertEquals(removeNullRecursively([]), []);
});

Deno.test("expect removeNullRecursively to handle complex nested structures", () => {
  const obj = {
    features: null,
    fieldToggle: null,
    variables: {
